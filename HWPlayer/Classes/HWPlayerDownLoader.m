...
@interface HWPlayerDownLoader()<NSURLSessionDataDelegate>
...
@implementation HWPlayerDownLoader
...
- (void)cancelAndClean {
    ...
    self.session = nil;
    // 清空本地已经存储的临时缓存
    [HWFileManager clearTmpFile:self.url];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler {
    ...
    self.outputStream = [NSOutputStream outputStreamToFileAtPath:[HWFileManager tmpFilePath:self.url] append:YES];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    ...
    //这句代码非常关键，他表示边下载边播放（告诉代理我此时正在把文件下载到内存，你可以做你的事了）
    if ([self.delegate respondsToSelector:@selector(downLoading)]) {
//        NSLog(@"self.delegate downLoading");
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    ...
    if (error == nil) {
        NSURL *url = self.url;
        if ([HWFileManager tmpFileSize:url] == self.totalSize) {
            //移动文件 : 临时文件夹 -> cache文件夹
            [HWFileManager moveTmpPathToCachePath:url];
        }
